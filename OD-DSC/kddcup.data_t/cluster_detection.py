...
class Cluster_Detection(object):
    ...
    def SLC(self):
        ...
        while len(index_out) != 0:
            ...
            if len(C) == 0:
                ...
                # center_index.append(indices)
                # C.append([])
                # C[j].append(indices)
                # index_out.remove(indices)
    ...
    def Detection(self, C, data_index):
        ...
        for i, c in enumerate(C):
            ...
            m += 1
            #去掉A_poi第一行的0
            print("168", C_A[:, self.data_dim], len(C_A))
        ...
        #self.R_threshold = 0.145
        # if len(A_con) != 0:
        #     self.R_threshold = self.get_RMSE(np.array(pre_X), pre_Y)*1.115
        #     #print("-----------------------------------", data_index)
